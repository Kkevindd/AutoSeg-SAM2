...
if __name__ == '__main__':
    ...
    parser.add_argument("--video_path", type=str, required=True, help="输入视频路径 (.mp4)")
    ...
    # 创建临时目录用于存放提取的帧
    temp_frames_dir = os.path.join(base_dir,'temp',video_name, "temp_frames")
    
    # 从视频中提取帧
    logger.info("从视频中提取帧...")
    frame_names, fps, width, height,frame_count = extract_frames(args.video_path, temp_frames_dir)
    video_dir = temp_frames_dir
    
    ##### 加载Sam2和Sam1模型 #####
    sam2_checkpoint = "/data/sam2/checkpoints/sam2_hiera_large.pt"
    ...
    sam_ckpt_path="/data/model_zoo/sam_vit_h_4b8939.pth"
    ...
    json_name = f"{video_name}_auto.json"
    ...
    # 生成分割视频
    #output_video_path = os.path.join(base_dir, f"segmented_video_{level}.mp4")
    #create_output_video(save_dir, temp_frames_dir, output_video_path, fps, width, height)
    
    # 生成仅掩码的视频
    mask_only_video_path = os.path.join(base_dir, 'semantic',f"{video_name}.avi")
    # 使用FFV1编码实现无损压缩（注意：部分系统可能需要安装对应编解码器）
    fourcc = cv2.VideoWriter_fourcc(*'FFV1')  
    # 创建VideoWriter时显式指定isColor=True
    mask_video = cv2.VideoWriter(mask_only_video_path, fourcc, fps, (width, height), isColor=True)
    ...
    # logger.info(f"处理完成! 分割视频已保存至: {output_video_path}")
